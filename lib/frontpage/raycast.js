...
function loadCssColors(){
  ...
  try {
    ...
    return {
      ...
      grey1: new THREE.Color(resolve('--color-drawing-grey1', fallbacks.grey1)),
      grey2: new THREE.Color(resolve('--color-drawing-grey2', fallbacks.grey2)),
      brown: new THREE.Color(resolve('--color-drawing-brown', fallbacks.brown))
    };
  } catch (err) {
    return {
...
async function loadLevelPNGFromOverlay(){
  ...
  if (!el) return Promise.reject(new Error('No #frontpage-background-overlay <img> found'));
...
const COLOR_BRUSH_SCALE = [0.8, 0.4, 0.35, 0.35, 1];
const COLOR_SEQUENCE = [0, 1, 0, 2, 0, 1, 0, 3, 4]; // pink, blue, pink, grey1, pink, blue, pink, grey2, brown
const COLOR_SEGMENT_MULTIPLIER = [1, 1, 1, 1, 1, 1, 1, 1, 2]; // make brown twice as long
const COLOR_AIR = [1, 1, 1, 1, 1];
const COLOR_FADE_PX = 100;

const scratchColor = new THREE.Color();

function getColorByIndex(idx){
  switch (idx) {
    case 0: return pigmentColors.pink;
    case 1: return pigmentColors.blue;
    case 2: return pigmentColors.grey1;
    case 3: return pigmentColors.grey2;
    case 4: return pigmentColors.brown;
    default: return pigmentColors.pink;
  }
}
...
function computeColorBlendState(){
  const segLen = segmentLengthFor(currentSequenceIndex);
  const nextSeqIdx = nextSequenceIndex(1);
  const nextColorIdx = COLOR_SEQUENCE[nextSeqIdx];
  if (segLen <= 1e-6) {
    return { mix: 0, nextColorIndex: nextColorIdx, segmentLength: segLen, traveled: 0 };
  }
  const traveled = segLen - segmentRemaining;
  if (nextColorIdx === currentColorIndex || COLOR_FADE_PX <= 0) {
    return { mix: 0, nextColorIndex: nextColorIdx, segmentLength: segLen, traveled };
  }
  const fadeWindow = Math.min(COLOR_FADE_PX, segLen);
  const fadeStart = Math.max(segLen - COLOR_FADE_PX, 0);
  if (traveled <= fadeStart) {
    return { mix: 0, nextColorIndex: nextColorIdx, segmentLength: segLen, traveled };
  }
  const denom = Math.max(fadeWindow, 1e-6);
  const mix = THREE.MathUtils.clamp((traveled - fadeStart) / denom, 0, 1);
  return { mix, nextColorIndex: nextColorIdx, segmentLength: segLen, traveled };
}
...
let currentBrushScale = COLOR_BRUSH_SCALE[currentColorIndex];
let segmentRemaining = 0;

function segmentLengthFor(seqIndex){
  const mult = COLOR_SEGMENT_MULTIPLIER[seqIndex] ?? 1;
  return COLOR_SEGMENT_PX * Math.max(mult, 0.0001);
}
...
function setSequenceIndex(index){
  ...
  currentBrushScale = COLOR_BRUSH_SCALE[currentColorIndex] ?? 1.0;
  segmentRemaining = segmentLengthFor(nextSeq);
...
function applyDynamicsFromSpeed(speed){
  ...
  const nextScale = COLOR_BRUSH_SCALE[blendState.nextColorIndex] ?? currentBrushScale;
  const scaleBlend = THREE.MathUtils.lerp(currentBrushScale, nextScale, blendState.mix);
  const baseSize = THREE.MathUtils.lerp(SIZE_MAX, SIZE_MIN, n) * scaleBlend;
  const segLen = blendState.segmentLength;
  const progress = segLen > 1e-6 ? 1 - (segmentRemaining / segLen) : 0;
  const oscillation = Math.max(0.2, 0.85 + 0.2 * Math.sin(progress * Math.PI * 2) + 0.3 * progress);
...
function setBrushFromEvent(e){
  ...
  if (lastX!==null && lastY!==null){
    let remainingStep = Math.hypot(clientX-lastX, clientY-lastY);
    while (remainingStep > 0){
      if (segmentRemaining <= 1e-6){
        setSequenceIndex(currentSequenceIndex + 1);
        continue;
      }
      if (remainingStep < segmentRemaining){
        segmentRemaining -= remainingStep;
        remainingStep = 0;
      } else {
        remainingStep -= segmentRemaining;
        setSequenceIndex(currentSequenceIndex + 1);
      }
    }
    segmentRemaining = Math.max(segmentRemaining, 0);
  }
...
const handlePointerDown = (e) => {
  ...
  segmentRemaining = segmentLengthFor(currentSequenceIndex);
  applyColorUniforms(computeColorBlendState());
};
...
const boot = async () => {
  ...
  if (!disposed && document.visibilityState === 'visible') startLoop();
};
...
return () => {
  ...
  window.removeEventListener('pointermove', handlePointerMove, passivePointerOpts);
  window.removeEventListener('pointerdown', handlePointerDown, passivePointerOpts);
  ...
  if (renderer && renderer.domElement && renderer.domElement.parentElement === appEl) {
