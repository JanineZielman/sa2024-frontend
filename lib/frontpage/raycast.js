...
function computeColorBlendState(){
  ...
  if (duration <= 1e-6) {
    return { mix: 0, nextColorIndex: nextColorIdx, segmentDuration: duration, elapsed: 0 };
  }
  const elapsed = duration - segmentRemainingMs;
  if (nextColorIdx === currentColorIndex || COLOR_FADE_MS <= 0) {
    return { mix: 0, nextColorIndex: nextColorIdx, segmentDuration: duration, elapsed };
  }
  ...
  return { mix, nextColorIndex: nextColorIdx, segmentDuration: duration, elapsed };
}
...
let colorTimerLastTickMs = (typeof performance !== 'undefined' ? performance.now() : Date.now());
...
function setBrushFromEvent(e){
  ...
  const moved = (prevX!==null && prevY!==null) ? Math.hypot(clientX-prevX, clientY-prevY) : 0;
  if (moved > 0) {
...
function tick(){
  ...
  if (colorTimerActive && (nowMs - lastMovementTimestamp) > COLOR_TIMER_IDLE_TIMEOUT_MS) {
    colorTimerActive = false;
  }
  if (DEBUG_RAYCAST && colorTimerActive !== debugPrevTimerActive) {
    console.log('[raycast]', 'timer', colorTimerActive ? 'started' : 'stopped');
